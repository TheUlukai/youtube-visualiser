...
- **Near-black accent colour in accentMap making nav buttons invisible** — `parse_sections.py` occasionally outputs a background colour (e.g. `#1A1A2E`, `#0a0a0f`) as a section's `accent_color` instead of the actual mid-range visual accent. Since `accentMap` drives the nav bar's Prev/Next button colours and tab highlights, a near-black accent makes those elements invisible against the dark nav background. Check: `assemble.py` now prints a WARNING for any accent with perceived brightness below 60. Fix by updating the `accentMap` entry to the section's real accent hex (found by reading the section component's primary colour constant).
